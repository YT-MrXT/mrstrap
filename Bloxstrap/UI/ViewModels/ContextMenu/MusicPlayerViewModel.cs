    ...
    Task.Run(() =>
    {
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = "Mrstrap Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "voidstrap_logo",
                        LargeImageText = "Mrstrap Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in Mrstrap{loopText}";
